...
type Item<T extends object = object> = {
  // 遺物効果ID
  // - 子項目がない場合は自身のID
  // - 子項目がある場合は子のIDをカンマ区切りで連結
  // - 選択できない項目の場合は連番で採番
  id: string
...
const inc = increment()
const allItems = Object.entries(relicEffectMap).map(([id, data]) => ({ id, ...data }))
 ...
 * @param name - 武器種名
 */
function weaponOptions(name: string) {
  return options(
    `${name}の攻撃力上昇`,
    `${name}の攻撃でHP回復`,
    `${name}の攻撃でFP回復`,
 ...
 * @param name - 遺物効果名
 */
function createOptionFromName(name: string): {
  id: string
  name: string
  stackable: boolean
  unselectable?: boolean
  ...
  const stackable = relatedItems.some((item) => item.stackable)
  ...
  return {
    ...
    children: relatedItems.map((item) =>
      item.name === name
        ? { id: exactMatch.id, name: item.name, stackable: item.stackable }
 ...
 */
function findRelatedItems(namePrefix: string) {
